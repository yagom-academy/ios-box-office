...
final class BoxOfficeJsonDecoderTests: XCTestCase {
    ...
    func tests_loadJsonData호출시_json데이터가_BoxOfficeResult로파싱된다() {
        ...
        // when, then
        do {
            let result = try sut.loadJsonData(name: assetName, type: BoxOfficeResult.self)
    ...
    func tests_loadJsonData호출시_파일이름이_잘못되었다면_DataAssetError를_반환한다() {
        ...
        // when, then
        XCTAssertThrowsError(try sut.loadJsonData(name: assetName, type: BoxOfficeResult.self)) {
            thrownError = $0
        }
