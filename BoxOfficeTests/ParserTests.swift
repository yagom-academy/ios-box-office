...
final class ParserTests: XCTestCase {
    ...
    func test_Pareser매서드로_boxOfficeType을_불러오면_일별_박스오피스가_결과로_반환된다() {
        //given
        let expectedResult = "일별 박스오피스"
        
        //when
        let result = sut.Parse(jsonFileName: "box_office_sample")?.boxOfficeResult.boxOfficeType
    ...
    func test_boxOfficeParser매서드로_DailyBoxOfficeList의_2번째배열의_movieName을_불러오면_스파이더맨노웨이홈이_결과로_반환된다() {
        ...
        //when
        let result = sut.Parse(jsonFileName: "box_office_sample")?.boxOfficeResult.dailyBoxOfficeList[1].movieName
